...
def handle_uploaded_file(file, data, key):
    if file:
        ...
        file_url = f'https://citizen-information-production.up.railway.app/get_photo/uploads{os.path.basename(file_path)}'
        file.save(file_path)
        data[key] = file_path
...
def get_saved_data():
    try:
        ...
        for data in filtered_data:
            if 'image' in data:
                data['image_url'] = f'https://citizen-information-production.up.railway.app/get_photo/{os.path.basename(data["image"])}'
